...
if __name__ == "__main__":
    ...
    # Have all nodes propose the same secret
    ...
    for i in range(1, n + 1):
        ...
        print(f"  Node {i}: {decision} [{status}]")
    
    # Check consensus
    decisions = [node.final_decision for node in NODES.values() if node.final_decision is not None]
    if len(set(decisions)) <= 1:
        print(f"\n✓ Consensus achieved: {decisions[0] if decisions else 'All nodes failed'}")
    else:
        print(f"\n✗ Consensus failed: Multiple different decisions")